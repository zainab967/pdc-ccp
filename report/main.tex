...
\title{Temperature Evolution Simulation of a Rod\\Using Parallel Computing Techniques}

\author{
    \IEEEauthorblockN{Muhammad Usman Butt}
    \IEEEauthorblockA{Department of Computer Science\\
    University of Management and Technology\\
    Lahore, Pakistan\\
    buttu7666@gmail.com}
    \and
    \IEEEauthorblockN{Nida Irshad}
    \IEEEauthorblockA{Department of Computer Science\\
    University of Management and Technology\\
    Lahore, Pakistan\\
    nidairshad66@gmail.com}
}
...
% ================================
\begin{abstract}
This report presents the implementation and performance analysis of parallel solutions for simulating temperature evolution in a one-dimensional rod using the heat equation. We implement four versions: sequential, OpenMP (shared memory), MPI (distributed memory), and CUDA (GPU). The Forward-Time Central-Space (FTCS) finite difference method is used for numerical discretization. Performance analysis with N=10,000 grid points reveals that OpenMP achieves a 1.57x speedup, while MPI and CUDA exhibit overhead limitations for this iterative problem. Results are validated against the analytical solution with L2 error of $3.82 \times 10^{-8}$.
...
\begin{itemize}
    ...
    \item GPU: NVIDIA GeForce RTX 3070 Laptop GPU
...
\midrule
Sequential     & 1             & 5,365.21  & 1.00x   \\
\textbf{OpenMP}& \textbf{4}   & \textbf{3,414.67} & \textbf{1.57x} \\
MPI            & 4             & 7,610.16  & 0.70x   \\
CUDA           & GPU           & 11,572.19 & 0.46x   \\
\bottomrule
\end{tabular}
\end{table}
...
Our results show that OpenMP achieves the best performance for this problem. The MPI and CUDA implementations are slower than sequential due to overhead:

\begin{itemize}
    \item \textbf{OpenMP (1.57x speedup):} Low thread synchronization overhead makes it ideal for this compute-bound problem.
    \item \textbf{MPI (0.70x):} Inter-process communication overhead on a single machine dominates. MPI excels on distributed clusters.
    \item \textbf{CUDA (0.46x):} With 1.25 million kernel launches (one per time step), kernel launch overhead ($\sim$5-10$\mu$s each) accumulates significantly.
\end{itemize}
...
\begin{itemize}
    \item \textbf{OpenMP achieved 1.57x speedup} with 4 threads, making it the best choice for this iterative problem on shared-memory systems
    \item \textbf{MPI is better suited for distributed clusters} rather than single-machine execution where communication overhead dominates
...
\textbf{Muhammad Usman Butt} is a Computer Science student at UMT Lahore specializing in parallel computing and high-performance systems.

\textbf{Nida Irshad} is a Computer Science student at UMT Lahore with interests in GPU programming and numerical methods.

\vspace{1em}
\textbf{YouTube Video:} [To be added after recording]
